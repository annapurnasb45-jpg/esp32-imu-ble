...
#define TOTAL_IMU_BYTES         (SAMPLE_RATE_HZ * SAMPLE_BYTES * DURATION_SECONDS) /* 4800 */

#define ATT_PAYLOAD_BYTES       244
#define SEQ_NUM_BYTES           2
#define IMU_CHUNK_BYTES         (ATT_PAYLOAD_BYTES - SEQ_NUM_BYTES)                /* 242 */
...
static void add_event_char(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to add event char: %s", esp_err_to_name(err));
    }
}
...
static void add_imu_char(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to add imu char: %s", esp_err_to_name(err));
    }
}
    ...
    esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GATTS_REG_EVT: {
            ...
            esp_err_t err = esp_ble_gatts_create_service(
                gatts_if,
                &service_id,
                GATTS_NUM_HANDLE
            );
        ...
        case ESP_GATTS_CREATE_EVT: {
            if (param->create.status != ESP_GATT_OK) {
                ESP_LOGE(TAG, "Service create failed, status=%d", param->create.status);
                break;
            }
            ...
            if (err != ESP_OK) {
                ESP_LOGE(TAG, "Start service failed: %s", esp_err_to_name(err));
                break;
            }
        ...
        case ESP_GATTS_ADD_CHAR_EVT: {
            if (param->add_char.status != ESP_GATT_OK) {
                ESP_LOGE(TAG, "Add char failed, status=%d", param->add_char.status);
                break;
            }

            if (gatt_step == STEP_ADD_EVENT_CHAR) {
                event_char_handle = param->add_char.attr_handle;
                ESP_LOGI(TAG, "Event char handle=%d", event_char_handle);
                add_event_cccd();
            } else if (gatt_step == STEP_ADD_IMU_CHAR) {
                imu_char_handle = param->add_char.attr_handle;
                ESP_LOGI(TAG, "IMU char handle=%d", imu_char_handle);
                add_imu_cccd();
            }
            break;
        }
        ...
        case ESP_GATTS_WRITE_EVT:
            if (param->write.len >= 2) {
                ...
                if (param->write.handle == event_cccd_handle) {
                    ...
                    ESP_LOGI(TAG, "Event notify %s",
                             event_notify_enabled ? "enabled" : "disabled");
                } else if (param->write.handle == imu_cccd_handle) {
                    imu_notify_enabled = (val == 0x0001);
                    ESP_LOGI(TAG, "IMU notify %s",
...
static void send_event_notification(uint8_t event_type)
{
    ...
    packet[0] = event_type;
    memcpy(&packet[1], &timestamp_ms, 4);
    packet[5] = confidence;
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG,
                 "Event sent: type=0x%02X timestamp=%lu confidence=%u",
                 event_type, (unsigned long)timestamp_ms, confidence);
    } else {
...
static void send_imu_notifications(void)
{
    ...
    ESP_LOGI(TAG, "Sending %d IMU packets", TOTAL_IMU_PACKETS);

    for (uint16_t seq = 0; seq < TOTAL_IMU_PACKETS; seq++) {
        size_t src_offset = seq * IMU_CHUNK_BYTES;
        size_t remaining = TOTAL_IMU_BYTES - src_offset;
        ...
        memcpy(&packet[2], &dummy_imu_data[src_offset], chunk);
    ...
    while (1) {
        ...
        if (last_level == 1 && level == 0) {
            ...
            if (gpio_get_level(BUTTON_PIN) == 0) {
                uint8_t event_type = random_event_type();

                ESP_LOGI(TAG, "Button pressed -> generating event 0x%02X", event_type);

                if (!device_connected) {
                    ESP_LOGW(TAG, "Not connected");
                } else if (!event_notify_enabled) {
                    ESP_LOGW(TAG, "Event notification not enabled");
                } else if (!imu_notify_enabled) {
                    ESP_LOGW(TAG, "IMU notification not enabled");
                } else if (negotiated_mtu < 247) {
                    ESP_LOGW(TAG, "MTU is %u, request MTU 247 in nRF Connect", negotiated_mtu);
                } else {
                    fill_dummy_imu_data();
                    send_event_notification(event_type);
